...
colors_strings2 = {'red', 'green', 'blue', 'yellow', 'orange', 'white'};
...
try
  for ii = 1:length(trialInfo)
    ...
    if trialInfo(ii, 1) == 1  % 1 is the congruent case
      ...
      tempColorsStr = colors_strings2;  %make a temp array which will delete the color that was originally inside
      tempColorsStr(trialColor) = [];
      thisColor = randi(length(tempColorsStr));
      cell2mat(cellfun(@(x) strcmp(colors_strings{thisColor,x), colors_strings2,'UniformOutput', false));
